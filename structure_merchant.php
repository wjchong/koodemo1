...
-->
                    ...
                    <div class="col-md-12">
						...
						?>
						 ...
						 <input onclick="window.location.href = 'dashboard.php';" type="button" class="btn btn-block btn-primary col-md-5" name="Wallet" value="<?php if($_SESSION['login'] && $u_bal>0){ echo strtoupper($wallet_name)." ".number_format($u_bal,2);} else{echo $language['wallet'];} ?>">
						 <input onclick='transfer("<?php echo $merchant_detail['mobile_number'];?>","<?php echo $merchant_detail['mobile_number']?>","<?php echo $merchant_detail['name']?>")' type="button" class="btn btn-block btn-primary col-md-5 trasfer" name="Wallet" value="<?php echo $language['tranfer']." ".$merchant_detail['mobile_number']; ?>">
						  
						<?php if($totalpartner>0){ ?>
						  <input  type="button" class="btn btn-block btn-primary col-md-5 partner"  value="<?php echo  $language['ASSOCIATED_PARTNER']; ?>">
						 <?php } ?>
						  <?php if($ppartner>0){ ?>
						  <input onclick="window.location.href = 'coinpartner.php?m_id=<?php echo $product['id'];?>';" type="button" class="btn btn-block btn-primary col-md-5"  value="<?php  echo $language['ACCEPT']." ".strtoupper($product['special_coin_name']); ?>">
						 <?php } ?>
						...
						<a class="btn btn-block btn-primary col-md-5" target="_blank" href="http://maps.google.com/maps?q=<?php echo  $merchant_detail['google_map']; ?>"> <?php echo $language['location']; ?></a> 
						
						<?php  } else { ?>
						<a class="btn btn-block btn-primary col-md-5"  href="#"> <?php echo $language['location']; ?></a> 
								      ...
								      while($prow = mysqli_fetch_assoc($u_query))
										{
											...
											?>
											  <li class="list-group-item" style="border-bottom: 1px solid #eaebeb;">
												...
												<div class="col-md-4">
												    <a class="btn btn-primary" style="color:white;max-height: 40px;" href="<?php echo $site_url."/structure_merchant.php?merchant_id=".$prow['merchant_id'];?>">Order  Now</a>
												   </br>
