            ...
            <div class="row page-title clearfix">
                ...
                <div class="page-title-right d-inline-flex">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><?php if(count($edit_data) > 0) {  echo "Update Assistant"; ?>  <?php }else{ echo "Create Assistant";} ?></li>
                    </ol>
                </div>
                <!-- /.page-title-right -->
            </div>
                    ...
                    <div class="col-md-12 widget-holder">
						...
						<form action="#" method="post"> 
                            <div class="widget-body clearfix">
                                <h5 class="box-title mr-b-0"><?php 
								//print_r($edit_data);
								
								
								if(count($edit_data) > 0) {  echo "Update Company Assistant"; ?>  <?php }else{ echo "Create Company Assistant";} ?></h5>
                                 ...
                                 <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            <div class="input-group">
                                                <input class="form-control" id="username" placeholder="Username" <?php if(count($edit_data) > 0) {?>  value="<?php echo $edit_data[0]['username'] ?>" <?php }else{ } ?> type="text">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            <input class="form-control"  id="email" placeholder="E-mail Address" <?php if(count($edit_data) > 0) {?>  value="<?php echo $edit_data[0]['email'] ?>" <?php }else{ } ?> type="email"> 
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            <input class="form-control" id="password" placeholder="Passsword"  type="text" <?php if(count($edit_data) > 0) {?>  value="<?php echo doDecode($edit_data[0]['password']) ?>" <?php }else{ } ?>>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            <input class="form-control" id="contactno" placeholder="Contact No" <?php if(count($edit_data) > 0) {?>  value="<?php echo $edit_data[0]['contact_no'] ?>" <?php }else{ } ?> type="text"> 
                                        </div>
                                    </div>
								...
								if(count($edit_data) > 0) {?> onclick="update_assistant()"  <?php }else{?> onclick="create_assistant()" <?php } ?>><?php 
								//print_r($edit_data);
								
								
								if(count($edit_data) > 0) {  echo "Update Assistant"; ?>  <?php }else{ echo "Create Assistant";} ?></a>
                                            </div>
                                        </div>
                                    </div>
                            </div>
...
function create_assistant()
{
	...
	var address = $('#address').val();
	 ...
	 if(usertype==""){
		...
		$.ajax({
		  method: "POST",
		  url: "<?php echo base_url();?>admin/Assistant/add_assistant",
		  data: {usertype:usertype,username:username,email:email,password:password,contactno:contactno,address:address}
		})
		.done(function(msg) {
						swal({
                title: "Insert", 
                text: "Assistant has been successfully Added", 
                icon: "success",
            }).then(function() {
               window.location = "<?php echo base_url();?>admin/Assistant/manage_assistant";
            });
...
function update_assistant()
{
	...
	var id = $('#ids').val();
	 ...
	 if(usertype==""){
		...
		$.ajax({
		  method: "POST",
		  url: "<?php echo base_url();?>admin/Assistant/update_assistant",
		  data: {usertype:usertype,username:username,email:email,password:password,contactno:contactno,address:address,id:id}
		})
		.done(function(msg) {
						swal({
                title: "update", 
                text: "Assistant has been successfully Updated", 
                icon: "success",
            }).then(function() {
               window.location = "<?php echo base_url();?>admin/Assistant/manage_assistant";
            });
