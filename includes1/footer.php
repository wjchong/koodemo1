   ...
   width: 260px!important;
	...
	<script src="https://rawgit.com/schmich/instascan-builds/master/instascan.min.js"></script>
      ...
      scanner.addListener('scan', function (content) {
        alert(content);   
		 var index = content.indexOf("60");    
		if(index !== -1){
			$('#transfer_to').val(content);
		...
		{ 
			...
			$('#qr_scan_model').modal('hide');
			$('#fund_wallet_input_modal').modal('show');
			
		} else{
			alert("Wrong Mobile No Scan Again!");
		}  
      });
	...
	$('.merchant_select').click(function(e) {
		...
		<?php 
		       $login_user_id=$_SESSION['login'];
				$qMerchant = mysqli_query($conn, "SELECT id,name FROM users WHERE name!='' and id not in('1944',$login_user_id)  ORDER BY name ASC");
