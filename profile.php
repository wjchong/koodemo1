...
if(isset($_POST['submit']))
{
	...
	if($flag == false)
	{
		mysqli_query($conn, "UPDATE users SET bank_name='$bankName',referred_by='$referral_id', real_name='$realname', mric_no='$mric_number', address='$address',email='$email', number_lock='$number_lock', facebook='$facebook', authentication='$authentication', bank_branch='$branchName', bank_ifsc='$ifsc_code', bank_ac_num='$ac_num', doc_copy='$filename', created_at='$date', account_type='$account_type', k_date='$k_date', k_lock='$k_lock' WHERE id='".$_SESSION['login']."'");
