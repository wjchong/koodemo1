...
{
	$Merchantid=$profile_data['parentid'];
}
else
{
...
elseif (!empty($tab))
{
// echo "SELECT * FROM order_list WHERE table_type='$tab' and section_type='$sec' and merchant_id ='$Merchantid'";
// die;
$data = mysqli_query($conn, "SELECT * FROM order_list WHERE id='$id'  and merchant_id ='$Merchantid'");
    $array_detail = array();
    while ($row=mysqli_fetch_assoc($data)){
        $user_id = $row['user_id'];
         $section_type = $row['section_type'];
